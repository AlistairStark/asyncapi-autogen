...
from typing import Any, Type, is_typeddict
...
class AsyncApiSchemaBuilder:
    ...
    def _get_field_data(self, f_type: Any) -> dict:
        ...
        if f_type is str:
            ...
            field_info = {
                "type": "array",
                "items": self._get_field_data(f_type.__args__[0]),
            }
        elif (
...
def generate_asyncapi_spec(
    ...
    for t in topic_list:
        if t.TOPIC in topic_names[t.TOPIC]:
            raise ValueError(f"Topic name {t.TOPIC} already exists. Did you register it twice?")
        topic_names[t.TOPIC].append(DOC_TYPE)
        schemas[t.__name__] = AsyncApiSchemaBuilder.build(t)
        channels[t.TOPIC][DOC_TYPE] = {
